qx.Class.define('joomtu.model.RemoteTable', {
	...
	members : {
		...
		__loadPHPRowCount : function() {
			...
			this.__call(param, function(data) {this._onRowCountLoaded(parseInt(data));});
		},

		__loadPHPRowData : function(firstRow, lastRow) {
			var param = "method=getRowData&start=" + firstRow + "&end=" + lastRow;
			this.__call(param, function(data) {this._onRowDataLoaded(qx.lang.Json.parse(data));});
		},

		__call : function(param, callback) {
			var req = new qx.io.request.Xhr();
			req.setUrl("http://127.0.0.1/qooxdoo/application/demobrowser/build/resource/demobrowser/backend/remote_table.php?" + param);
			req.addListener("success", function() {callback.call(this, req.getResponseText())}, this);
			req.send();
		},
		...
		__setRowCount : function() {
			var self = this;
			window.setTimeout(function() {self._onRowCountLoaded(1000000);}, 0);
		},
