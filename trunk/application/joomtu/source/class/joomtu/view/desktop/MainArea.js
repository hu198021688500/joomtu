qx.Class.define("joomtu.view.desktop.MainArea", {
	...
	construct : function() {
		...
		this.setOrientation("vertical");
			...
			// var tableModel = new
			// joomtu.model.TablePaging(this.__config.uiConfig.user_list);
			...
			/*
			 * tableModel.addListener("dataChanged", function(evt){ var
			 * data = evt.getData();
			 * //this.__table.setAdditionalStatusBarText(data.firstRow + "
			 * to " + data.lastRow + " " + this.getRowCount()); });
			 */
			// this.__table = new qx.ui.table.Table(tableModel, custom);
			this.__table = new joomtu.view.desktop.PagingTable(
					tableModel, custom);
			var col = this.__table.getTableColumnModel().getBehavior();
			col.setWidth(0, '10%');
			col.setWidth(1, '90%');

			// Establish a context menu for the boolean field, to select
			// its value
			// this.__table.setContextMenuHandler(1,
			// this.__contextMenuHandlerBoolean);

			// add table
			/*
			 * var tableModel = new qx.ui.table.model.Simple();
			 * tableModel.setColumns(["ID", "A number", "A date",
			 * "Boolean"]);
			 ...
			 * qx.ui.table.headerrenderer.Icon("icon/16/apps/office-calendar.png",
			 * "A date")); this.__table.setFocusedCell(2,5);
			 */
		...
		__contextMenuHandlerBoolean : function(col, row, table,
			...
			for (var i = 0; i <= 1; i++) {
				...
				menuEntry.addListener("execute", function(evt) {
							// Toggle the value
							dataModel.setValue(col, row, this
											.getUserData("value"));
						});
		...
		__createRandomRows : function(rowCount) {
			...
			for (var row = 0; row < rowCount; row++) {
				var date = new Date(now + Math.random() * dateRange
						- dateRange / 2);
				rowData.push([this.__nextId++, Math.random() * 10000,
